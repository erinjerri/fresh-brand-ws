...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  // Safely drop existing enum type first
  await db.execute(sql`
    DO $$
    BEGIN
      -- Check if the enum type exists
      IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_pages_hero_links_link_type') THEN
        -- Drop the enum type if it exists
        DROP TYPE "enum_pages_hero_links_link_type";
      END IF;
    END
    $$;
  `);
  ...
  await db.execute(sql`
    ...
    BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_pages_hero_links_link_type') THEN
        CREATE TYPE "enum_pages_hero_links_link_type" AS ENUM ('primary', 'secondary');
      END IF;
    END
    $$;
  `);
  ...
  CREATE TYPE "public"."enum_footer_nav_items_category" AS ENUM('About', 'Experience', 'Read', 'Watch', 'Buy');
  ...
  CREATE TABLE IF NOT EXISTS "footer_nav_items" (
  	...
  	"link_url" varchar,
  	"link_label" varchar NOT NULL,
  	"category" "enum_footer_nav_items_category" NOT NULL
  );
  ...
  await db.execute(sql`
    DO $$ BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_type WHERE typname = 'enum_pages_hero_links_link_type'
      ) THEN
        CREATE TYPE "enum_pages_hero_links_link_type" AS ENUM ('primary', 'secondary');
      END IF;
    END $$;
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TYPE "public"."enum_footer_nav_items_category";`);
}
